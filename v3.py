...
from Crypto.Util.Padding import pad
import webbrowser
from NFT import deploy_and_mint_nft
    ...
    if original_file:
        ...
        if response.status_code == 200:
            ...
            if ipfs_link:
                ...
                privatekey=st.text_input("Enter Private key")
                #st.write(wallet)
                if st.button(label="Create NFT"):
                     url = ipfs_link_dict.get('ipfs_storage', {}).get('ipfs_url', 'No URL found')
                     k=deploy_and_mint_nft(url)
                     st.success(f"NFT Minted ! Contract deployed at\n : {k}")
